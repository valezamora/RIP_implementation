...
def compartir():
    ...
    if (threading.current_thread().getName() == 'comparte3'):
        while (count < 3):
            print('COMPARTE3')
            # Send tablaRutas3
            sent = sockets[3][0].sendto("Test 3".encode(), (HOST,principal_port))
            count += 1

    else:
        while (count < 10):
            if (threading.current_thread().getName() == 'comparte1'):
                sent = sockets[1][0].sendto("Prueba 1".encode(), (HOST, principal_port))
                print('COMPARTE1')
                # Send tablaRutas1
            elif (threading.current_thread().getName() == 'comparte2'):
                sent = sockets[2][0].sendto("Mensaje 2".encode(), (HOST, principal_port))
                print('COMPARTE2')
                # Send tablaRutas2
            elif (threading.current_thread().getName() == 'comparte4'):
                sent = sockets[4][0].sendto("Hilera 4".encode(), (HOST, principal_port))
                print('COMPARTE4')
                # Send tablaRutas4
            elif (threading.current_thread().getName() == 'comparte5'):
                sent = sockets[5][0].sendto("Texto 5".encode(), (HOST, principal_port))
                print('COMPARTE5')
                # Send tablaRutas5
            count += 1
            time.sleep(3)
...
for num_hilo in range(1,6):
    ...
    grupo_recepcion = (HOST, principal_port)
